<template>
  <div class="about">
    ...
    <div class="input-group">
      <input type="search" class="form-control rounded" placeholder="Suchen" aria-label="Search"
             aria-describedby="search-addon" v-model="inputText"/>
    </div>
  </div>
  <table class="table">
    ...
    <tbody slot="body">
    <tr v-for="element in filterContacts">
      <td>
        <font-awesome-icon icon="fa-solid fa-trash" @click="putInTrash(element.kid)"></font-awesome-icon>
      </td>
      <td>
        <font-awesome-icon icon="fa-solid fa-edit" @click="edit(element.kid)"></font-awesome-icon>
      </td>
  ...
  computed: {
    filterContacts() {
      return this.data.filter(item => {
        return item.name.toLowerCase().startsWith(this.inputText.toLowerCase()) || item.name.toLowerCase().includes(this.inputText.toLowerCase())
      });
    }
  },
}
